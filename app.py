...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'yas_ultimate_converter_pro_2026'
...
def convert_image_universal(img_stream, target_format, size=None):
    try:
        ...
        if size and size[0] > 0 and size[1] > 0:
            # Menggunakan LANCZOS untuk hasil downscale/upscale terbaik
...
def process_file(file_id, file_info, target_format, width=None, height=None):
    try:
        ...
        if is_zip:
                    ...
                    for name in z_in.namelist():
                        ...
                        if any(name.lower().endswith(x) for x in ['.png','.jpg','.jpeg','.webp','.bmp','.tga','.dds','.psd','.ico']):
                            try:
                                ...
                                conv = convert_image_universal(io.BytesIO(img_data), target_format, size)
                                if conv:
            ...
            conv = convert_image_universal(io.BytesIO(file_info['data']), target_format, size)
...
def convert():
    ...
    success = process_file(fid, info, fmt, w, h)
    if success:
